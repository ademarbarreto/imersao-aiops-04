...
Empacotar a aplicação kube-news em uma imagem Docker executável e fornecer um ambiente de desenvolvimento local via Docker Compose (app + Postgres), com build reprodutível, execução sem privilégios de root, healthchecks e uma fonte única de credenciais do banco compartilhada entre os serviços.
